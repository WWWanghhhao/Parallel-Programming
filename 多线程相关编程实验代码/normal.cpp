...
int main() {
  ...
  // for (int i = 0; i < 10; i++) {
  //   for (int j = 0; j < i; j++) {
  //     cout << clusters_dis[0][i][j] << " ";
  //   }
  //   cout << endl;
  // }
  // cout << clusters[0].size() << " " << clusters_dis[0].size() <<
  // "==============\n";
  vector<vector<int>> right_ans(10000, vector<int>(100));
  load_ivecs_data("../data/sift_groundtruth.ivecs", right_ans, num, dim);
  // auto res = compute_topK(id[0], clusters, clusters_dis, 100);
  // int cnt = 0;
  // for (int j = 0; j < res.size(); j++) {
  //   if (find(right_ans[0].begin(), right_ans[0].end(), res[j]) !=
  //       right_ans[0].end()) {
  //     cnt++;
  //   }
  // }
