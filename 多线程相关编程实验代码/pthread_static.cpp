...
int main() {
  ...
  for (int j = 100; j <= 10000; j += 100) {
    ...
    // if(j == 10000){
    //   ofstream ofss("res-col.txt",ios::out);
    //   for(int i = 0;i<100;i++){
    //     ofss << id[i][0] << endl;
    //   }
    //   ofss.close();
...
//     vector<float> v = vector<float>(ori_data[i].begin() + column_idx * 32,
//     ori_data[i].begin() + (column_idx + 1) * 32); sem_wait(&semaphore);
//     pthread_mutex_lock(&mutex);
...
//   sem_init(&semaphore, 0, 4); // Set the semaphore count to the number of
//   threads pthread_mutex_init(&mutex, NULL);
...
//     gettimeofday(&t2, NULL);
//     timeuse = (t2.tv_sec - t1.tv_sec) + (double)(t2.tv_usec - t1.tv_usec) /
//     1000000.0; printf("%.6f seconds \n", timeuse); times.push_back(timeuse);
//     // if(j == 10000){
//     //   cout << "=============\n";
//     //   ofstream ofss("res2.txt", ios::out);
//     //   for (int i = 0; i < id.size(); i++) {
