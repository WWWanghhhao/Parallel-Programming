...
float dist(vector<float> v1, vector<float> v2, DisType type) {
  ...
  float sum = 0.0;
  float m1 = 0, m2 = 0;
  int neonSize = v1.size();
  switch (type) {
  case EucDis: {
    float32x4_t sumVec = vdupq_n_f32(0.0);
    for (int i = 0; i < neonSize; i += 4) {
      float32x4_t v1Vec = vld1q_f32(&v1[i]);
      float32x4_t v2Vec = vld1q_f32(&v2[i]);
      float32x4_t diff = vsubq_f32(v1Vec, v2Vec);
      float32x4_t squared = vmulq_f32(diff, diff);
      sumVec = vaddq_f32(sumVec, squared);
    }
    float32x2_t sumPair =
        vpadd_f32(vget_low_f32(sumVec), vget_high_f32(sumVec));
    sum = vget_lane_f32(vpadd_f32(sumPair, sumPair), 0);
    for (int i = neonSize; i < size; i++) {
